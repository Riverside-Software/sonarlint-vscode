...
export class NewCodeDefinitionService {
  ...
  private updateStatusBarTooltip(newCodeDefinition: NewCodeDefinition) {
    ...
    if (newCodeDefinition && this.focusOnNewCode) {
      ...
      newCodeDefinitionMessage = 'All issues are shown.\n\nSet CABL focus on new code to see only issues in recently added or changed code. This will help you practice Clean as You Code.';
  ...
  updateStatusBarText() {
    ...
    this.newCodeStatusBarItem.text = `CABL focus: ${enabledText}`;
  }
  ...
  private getFocusOnNewCodeFromConfiguration() {
    return VSCode.workspace.getConfiguration('sonarlint-abl').get('focusOnNewCode', false);
  }
}
